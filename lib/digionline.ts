...
#########################################
          ...
          (response) => {
            this.checkLoggedIn((loggedIn) => {
              if (loggedIn) {
                ...
                Log.error(
                  `Sikertelen belepes (helyes a felhasznalonev es jelszo?)`,
                  "1"
                );
              }
              cb(loggedIn);
    ...
    Common.request(
      ...
      (response) => {
        ...
        if (dom.window.document.querySelector(".in-user")) {
          const loggedEmail = dom.window.document
            .querySelector(".in-user")
            .textContent.trim();
  ...
  public getChannelList(
    cb: (channelList: Array<ChannelInterface>) => void
  ): void {
    ...
    Common.request(
      ...
      (response) => {
        ...
        dom.window.document
          ...
          .forEach((channelBox) => {
            const name: string = channelBox
              .querySelector(".channels__name")
              .textContent.trim();
            const logoUrl: string = channelBox.querySelector("img").src;
            const id: number = Number(
              channelBox.querySelector(".favorite").getAttribute("data-id")
            );
            const categoryNumber: number = Number(
              channelBox.getAttribute("data-category")
            );
            const category: string =
              categoryNumber in categoryMapping
                ? categoryMapping[categoryNumber]
